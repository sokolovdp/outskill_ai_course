...
# Specialized agents for Flow routing
entry_strategist = Agent(
    role="Entry Strategy Specialist",
    goal="Create a detailed entry strategy for buying a stock",
    llm=llm,
    verbose=True,
    backstory=(
        "You are a specialist in timing stock purchases. You create actionable entry strategies "
        "including entry price ranges, position sizing suggestions, and timeline recommendations."
    ),
...
exit_planner = Agent(
    role="Exit Strategy Specialist",
    goal="Create a detailed exit plan for selling a stock",
    llm=llm,
    verbose=True,
    backstory=(
        "You are a specialist in risk management and exit strategies. You create actionable exit plans "
        "including timing recommendations, risk warnings, and alternative investment suggestions."
    ),
    tools=[get_current_stock_price, get_company_info],
    max_iter=4,
    max_rpm=10,
...
hold_monitor = Agent(
    ...
    goal="Create a monitoring plan for stocks on hold",
    llm=llm,
    verbose=True,
    backstory=(
        "You are a specialist in portfolio monitoring. You identify key indicators to watch, "
        "set re-evaluation triggers, and create a watchlist of events that could change the recommendation."
    ),
    tools=[get_current_stock_price, get_company_info],
    max_iter=4,
    max_rpm=10,
